	...
	
{
	...
	{ 
		$st_login_check = "select  * from st_info where st_username = '$st_username' and st_password=md5('$st_password')";
		$st_login_run = $this->connectdb->query($st_login_check);
		$st_login_num = $st_login_run->num_rows;
		return $st_login_num;
	}

	public function student_reset_pwd($st_username, $st_id, $st_password)
	{ 
		$st_reset_pwd = "UPDATE st_info SET st_password=md5('$st_password') WHERE st_id=$st_id AND st_username='$st_username'";
		$reset = $this->connectdb->query($st_reset_pwd);
	...
	public function student_info_fee_payment($st_id)
	{
		$student_info_sel = "select tblfeepayment.*, st_fullname, st_id from tblfeepayment LEFT JOIN st_info ON tblfeepayment.student_id = st_info.st_id where student_id=$st_id";
		$student_info_run = $this->connectdb->query($student_info_sel);
	...
	{
		$meadin_login_select = "select * from meadmin where admin_username='$admin_username' AND admin_password=md5('$admin_password')";
	...
	public function admin_reset_pwd($admin_username, $admin_id, $admin_password)
	{ 
		$admin_reset_pwd = "UPDATE meadmin SET admin_password=md5('$admin_password') WHERE id=$admin_id AND admin_username='$admin_username'";
		$reset = $this->connectdb->query($admin_reset_pwd);
	...
	{
		$student_password_update = "update st_info set st_password=md5('$st_password_update') where st_username='$st_username'";
	...
	public function update_teacher_info($up_fullname,$up_address,$up_email,$up_qualification,$up_contact,$up_staff,$up_gender,$teacher_id)
	{
		$update_teacher_info_select = "update teacher_info set t_fullname='$up_fullname',t_address='$up_address',t_email='$up_email',t_qualification='$up_qualification',t_contact='$up_contact',t_staff_type='$up_staff',t_gender='$up_gender' where t_id='$teacher_id'";
	...
	{
		$grade_info_select = "select * from sub_class_name ";
		$grade_info_run = $this->connectdb->query($grade_info_select);
		return $grade_info_run;
	}
	...
	{
		$add_student_insert = "insert into st_info(st_fullname,st_username,st_password,st_grade,roll_no,st_dob,st_address,st_district,st_gender,st_father,st_mother,st_parents_contact) value('$std_fullname','$std_username',md5('$std_password'),'$std_grade','$std_roll','$std_dob','$std_address','$std_district','$std_gender','$std_father','$std_mother','$std_parent_contact')";
	...
	// results
	public function add_result_info($teacher_id, $class_id, $subject_id, $student_id, $term, $year, $marks)
	{
		$query = "INSERT into tblresult(TeacherId, ClassId, SubjectId, StudentId,Term, Year, marks)";
		$query .= " VALUES($teacher_id, $class_id, $subject_id, $student_id, $term, $year, $marks)";


		$saved = $this->connectdb->query($query);
		return $saved;
	}

	public function result_info($query)
	{
		$results = $this->connectdb->query($query);
		return $results;
	}

	public function add_fee_info($class_id, $term, $year, $amount)
	{
		$query = "INSERT into tblfee(class_id, term, year, amount)";
		$query .= " VALUES($class_id, $term, $year, $amount)";
	...
	public function fee_info_display()
	{
		$results = $this->connectdb->query("SELECT * FROM tblfee");
		return $results;
	}

	public function fee_info($query)
	{
		$results = $this->connectdb->query($query);
		return $results;
	}
	...
	public function add_feepayment_info($student_id, $fee_id, $amount)
	{
		$query = "INSERT into tblfeepayment(student_id, fee_id, amount)";
		$query .= " VALUES($student_id, $fee_id, $amount)";
	...
	public function feepayment_info($query)
	{
		$results = $this->connectdb->query($query);
		return $results;
	}
